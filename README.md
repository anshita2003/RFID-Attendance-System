...
Each user scans their RFID card on the RFID reader connected to NodeMCU.
The card’s UID is detected using SPI communication.
NodeMCU connects to Wi-Fi and sends the UID to the PHP server hosted in XAMPP.
The PHP script checks the MySQL database:
...
Open phpMyAdmin → Create a new database named rfid_attendance.
Import the file database.sql.
The table attendance will store:
id (Auto Increment)
card_uid
name
timestamp
...
Open browser → Visit: http://localhost/RFID-Attendance-System/
Open Arduino IDE, load your NodeMCU code, and update:
